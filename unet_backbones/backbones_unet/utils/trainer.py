...
class Trainer:
    ...
    def trans_test(self, data_loader, model=None, model_name=None):
        ...
        model.cuda()
        model.eval()
        feature_path = os.path.join(self.results_path, '{}_{}'.format(model_name, 'feature_map'))
        ...
        if self.source_layer is not None:
            feature_layer = get_source_layer(model)[0]
        ...
        with tqdm(data_loader, unit=" validating-batch", colour="green") as evaluation:
            for i, (images, labels, name) in enumerate(evaluation):
                ...
                if '2007_001311' in name and self.source_layer is not None:
                    mid_original = torch.zeros(mid_output.size()).cuda()
                    mid_original.copy_(mid_output)  # original feature map

                    n, c, h, w = mid_original.size()
                    mid_original = torch.nn.functional.normalize(mid_original,
                                                                 dim=1)  # original feature map normalize
                    ...
                    for i in range(W.size(1)):
                        if i == 589:
                            pxl_cormap = torch.reshape(W[0][i], (h, w))
                            pxl_cormap_img = (pxl_cormap.detach().cpu().numpy() * 255).astype(np.uint8)
                            color_image = cv2.applyColorMap(pxl_cormap_img, cv2.COLORMAP_JET)
                            color_image = cv2.resize(color_image, (128, 128), interpolation=cv2.INTER_LINEAR)
                            cv2.imwrite(
                                os.path.join(feature_path, '{}_{}.png'.format(name[0], i)),
                                color_image)
                    del W
    ...
    def _evaluate(self, data_loader, epoch=None, model=None):
        ...
        model.cuda()
        model.eval()
        feature_path = os.path.join(self.results_path, '{}_{}'.format(self.target_model, 'feature_map'))
    ...
    def adv_attack(self, data_loader, epoch):
        ...
        with tqdm(data_loader, unit=" validating-batch", colour="green") as evaluation:
            for i, (images, labels, name) in enumerate(evaluation):
                ...
                if 'pgd' in self.attack or 'proposed' in self.attack:
                    if self.norm == 'inf':
                ...
                for t in range(self.iterations):
                    ...
                    elif self.attack == 'transegpgd':
                        loss = functions.transegpgd_scale(
                            predictions=preds,
                            labels=labels,
                            loss=loss,
                            iteration=t,
                    ...
                    elif self.attack == 'proposed':
                        loss = functions.proposed_scale(
