...
# * Save dir
    ...
    parser.add_argument('-at', '--attack', type=str, default='cospgd', choices={'fgsm', 'cospgd', 'segpgd', 'pgd', 'proposed', 'transegpgd'},
    ...
    parser.add_argument('-m', '--mode', type=str, default='adv_attack', choices={'adv_attack', 'adv_train', 'train', 'test', 'trans_test'},
                        help='What to do?')
...
def main(args):
    ...
    if args.mode == 'adv_attack':
        if args.attack == 'proposed':
            ...
            logger.info("---------- Method: {}, Source model-----------".format(args.attack,args.model))
    ...
    # Transferability
    models = ['deeplabv3_resnet50', 'deeplabv3_resnet101', 'psp_resnet50' ,'psp_resnet101', 'fcn8s_vgg16']
    ...
    for model_name in models:
        ...
        model_ = model_name.split('_')
        model = get_segmentation_model(model=model_[0], dataset=args.pretrained_data, backbone=model_[1],
                                   pretrained=True, root=args.pt).to(args.device)

        trainer.trans_test(val_loader, model=model, model_name=model_name)
